...
public class HybridLook : MonoBehaviour
{
  ...
  public void CameraInput(Vector2 c_Input)
  {
    ...
    if (m_Smooth)
    {
      HybridMotor.m_Character.localRotation = Quaternion.Slerp(character.localRotation, m_CharacterTargetRot,
          m_SmoothTime * Time.deltaTime);
      GetComponent<Camera>().localRotation = Quaternion.Slerp(camera.localRotation, m_CameraTargetRot,
          m_SmoothTime * Time.deltaTime);
    }
    else
    {
      character.localRotation = m_CharacterTargetRot;
      GetComponent<Camera>().localRotation = m_CameraTargetRot;
    }
