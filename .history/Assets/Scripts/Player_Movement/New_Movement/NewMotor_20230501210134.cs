...
public class NewMotor : MonoBehaviour {
  ...
  [SerializeField]
  private float m_SprintSpeed = 7;
  [SerializeField]
  private float m_CrouchSpeed = 3;
  ...
  public void isSprinting() {
    if (h_Man.g_Player.Sprint.IsPressed() && (m_Stamina > 1) &&
        m_MoveInput != Vector3.zero && m_hasRegened) {
      {
  ...
  // Air control occurs when the player is in the air, it allows players to move
  // side to side much faster rather than being 'sluggish' when it comes to
  // cornering.
  ...
  private void GroundMove() {
    ...
    if (m_CrouchQueued) {
      m_GroundSettings.MaxSpeed = m_CrouchSpeed;
    }

    if (m_isSprinting) {
      m_GroundSettings.MaxSpeed = m_SprintSpeed;
    }
  ...
  private void ApplyFriction(float t) {
    ...
    if (m_Character.isGrounded) {
      float control = speed < m_GroundSettings.Deceleration
